...
class _antialias_func(torch.autograd.Function):
    @staticmethod
    def forward(ctx, color, rast, pos, tri, sdf, topology_hash, pos_gradient_boost):
        out, bg_subpixel_tensor, work_buffer = _get_plugin().antialias_fwd(color, rast, pos, tri, sdf, topology_hash)
...
# Op wrapper.
def antialias(color, rast, pos, tri, sdf=None, topology_hash=None, pos_gradient_boost=1.0, bg_subpixel=False):
    ...
    # Instantiate the function.
    out, bg_subpixel_tensor = _antialias_func.apply(color, rast, pos, tri, sdf, topology_hash, pos_gradient_boost)
    if bg_subpixel:
        return out, bg_subpixel_tensor
    else:
        return out
